...
const SimilarMovies = ({ id }) => {
  ...
  const searchMovie = async () => {
    const url = `https://api.themoviedb.org/3/movie/${id}/similar?api_key=be1b59e50a18452a44eab0956cf0f5b4&language=tr-TR`;
      ...
      <div className="text-red-500">
        {loading ? (
          ...
          <div className="px-16 mt-4">
            <h2 className="rounded bg-white w-fit  px-16 py-2 font-bold">
              Benzer film bulunamadı!
            </h2>
          </div>
        ) : (
          <div className="mt-12 container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 place-items-center gap-20 px-6 md:px-[5.5rem]">
            {movies.map((data) => (
              <div
                key={data.id}
